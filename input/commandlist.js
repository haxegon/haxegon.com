...
["Gfx.resizescreen","(width, height)","F", "Sets the width and the height of the screen canvas. Automatically scales the screen to fit in the current window, depending on your resize settings. Advanced settings: If you set both width and height to 0, your screen size will instead match the window exactly, with Gfx.screenwidth and Gfx.screenheight changing to match. If you set EITHER width or height to 0, then you'll get a dynamic canvas, where one axis is streched and the other changes to match the window width."],
...
["Gfx.scale","(xscale:Float, yscale:Float, xpivot=Gfx.LEFT, ypivot=Gfx.TOP)","F","Scales image drawing functions like Gfx.drawimage and Gfx.drawtile. Optionally set scaling pivot point. Scales X and Y seperately - if you call with only one argument, it scales X and Y the same. If you call with no arguments, it resets the scale to 1.0."],
...
["Gfx.keeppixelratio",":Bool","P", "<b>false</b> by default - set to <b>true</b> to use black borders around your screen when scaling to keep a 1:1 pixel ratio, instead of stretching the screen. Good for low resolution pixely games!"],
...
["Filter.blur",":Float","P", "Turns on a fullscreen blur filter. You can set it to 1.0 for a normal blur effect, but any value will work."],
...
["Music.play","(songname:String, ?offsettime:Float = 0.0, ?fadeintime:Float = 0, ?loop:Bool = true, ?panning:Float = 0)","F", "Plays a song from <b>data/sounds/</b> folder. Optionally set a startpoint (where 1.0 is 1 seconds into the song), a fade in time (0 means start immediately, 1.0 means fade in for one second), whether or not to loop, and a panning value (between -1 and +1 for left to right)."],
...
["Sound.load","(soundname:String, volumelevel:Float = 1.0)","F", "Loads in a sound from <b>data/sounds/</b> - either MP3,OGG or WAV, depending on platform. See \"<b>how to add assets.txt</b>\" in your data folder for more details. Optionally set a volume level for the sound (0.5 loads it in at half volume, for example.)"],
["Sound.play","(soundname:String, ?offsettime:Float = 0.0, ?fadeintime:Float = 0, ?loop:Bool = false, ?panning:Float = 0)","F", "Plays a sound from <b>data/sounds/</b> folder. Optionally set a startpoint (where 1.0 is 1 seconds into the song), a fade in time (0 means start immediately, 1.0 means fade in for one second), whether or not to loop, and a panning value (between -1 and +1 for left to right)."],
